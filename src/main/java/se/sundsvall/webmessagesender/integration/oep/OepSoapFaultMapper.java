...
public class OepSoapFaultMapper {
	...
	/**
	 * Helper method for converting SOAPFaultException objects to a throwable problem. Incoming fault has the following structure:
	 * 
	 * <pre>
	 ...
	 * </pre>
	 * 
	 * which the method interprets and converts to a throwable problem. If detail contains a child with one of the known faults
	 * that can be thrown by OEP, a throwable problem containing corresponding status code will be returned. In all other cases
		...
		.forEachRemaining(entry -> {
			ThrowableProblem throwableProblem = 
				switch (entry.getLocalName()) {
					case ACCESS_DENIED_FAULT -> Problem.valueOf(UNAUTHORIZED, soapFaultException.getFault().getFaultString());
					case FLOW_INSTANCE_NOT_FOUND_FAULT -> Problem.valueOf(NOT_FOUND, soapFaultException.getFault().getFaultString());
					default -> null;
				};
			if (nonNull(throwableProblem)) wrapper.problem = throwableProblem;
		});
		
		return nonNull(wrapper.problem) ? 
				wrapper.problem : 
				Problem.valueOf(INTERNAL_SERVER_ERROR, soapFaultException.getFault().getFaultString());
	}
}
